...
import { Plus, ArrowLeft, Play, AlertCircle, CheckCircle, Clock } from "lucide-react";
...
export default function TopicDetails() {
  ...
  const fetchQuestions = async () => {
    if (!token) return;
    try {
      const res = await fetch(`http://localhost:5242/api/PromptIntelligence/topics/${topicId}/questions`, {
  ...
  const createQuestion = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5242/api/PromptIntelligence/questions", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}` 
        },
        body: JSON.stringify({ promptTopicId: topicId, promptText: newQuestionText })
      });
      if (res.ok) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto text-white">
      <Link href="/dashboard/prompt-intelligence" className="flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition-colors w-fit">
        <ArrowLeft size={16} />
        Back to Topics
      </Link>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-gray-400">Manage and analyze specific prompts within this topic.</p>
        </div>
        <button 
      ...
      <div className="bg-[#111318] border border-gray-800 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-gray-400 mb-6 max-w-md mx-auto">Add a prompt that your users might ask an AI (e.g. "What is the best CRM in 2026?").</p>
            <button 
              onClick={() => setIsModalOpen(true)}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-800 bg-[#0a0a0c]">
                ...
                <th className="px-6 py-4 font-medium text-gray-400 text-center">Last Run</th>
                <th className="px-6 py-4 font-medium text-gray-400 text-center">Visibility</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {questions.map((q) => {
                ...
                return (
                  <tr key={q.question.id} className="hover:bg-[#1a1c23] transition-colors group">
                    <td className="px-6 py-5">
                      <p className="font-medium text-white group-hover:text-blue-400 transition-colors cursor-pointer" 
                         onClick={() => router.push(`/dashboard/prompt-intelligence/${topicId}/analyze/${q.question.id}`)}>
                    ...
                    <td className="px-6 py-5 text-center">
                      {q.latestAnalysis ? (
                        <div className="flex items-center justify-center gap-2">
                          {isCompleted ? <CheckCircle size={14} className="text-emerald-500" /> : <Clock size={14} className="text-amber-500" />}
                          <span className="text-sm text-gray-300">
                            {new Date(q.latestAnalysis.runAt).toLocaleDateString()}
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-5 text-center">
                      {isCompleted ? (
                        <span className={`inline-flex items-center justify-center px-2.5 py-1 rounded-full text-sm font-medium ${
                          visibility > 70 ? 'bg-emerald-500/10 text-emerald-400' :
                          visibility > 30 ? 'bg-amber-500/10 text-amber-400' :
