...
export default function PromptsDashboard() {
  ...
  useEffect(() => {
    const fetchQuestionsForTopic = async (topicId: string) => {
      try {
        const res = await fetch(`http://localhost:8088/api/PromptIntelligence/topics/${topicId}/questions`, {
    ...
    const fetchTopics = async () => {
      if (!token) return;
      try {
        const res = await fetch("http://localhost:8088/api/PromptIntelligence/topics", {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
