...
export default function ReportCover({ data }: { data: FullReportData }) {
  ...
  const date = exec?.createdAt
    ? new Date(exec.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
      ...
      <div className="mt-10 md:mt-0 flex gap-6 relative z-10">
        ...
        <div className="text-center p-6 bg-blue-50 rounded-2xl border border-blue-100 min-w-[140px]">
          ...
          <p className="text-4xl font-black text-blue-700">
            {exec?.overallAIVisibilityScore || 0}
            <span className="text-lg text-blue-400 font-medium">/100</span>
          </p>
          <div className="mt-3 flex justify-center">
            <MetricProvenanceBadge kind="derived" />
          </div>
